        ...
        success: function(data){
            ...
            $.each(data.dataList,function(index,val){
                ...
                $('.recommend>button').click(function(){
                    console.log('aaaa');
                    var hongheibangdan = 'hongheibangdan';
                    $.ajax({
                        type: 'get',
                        url: 'http://59.152.38.197:8188/SmartHotelInterface/api/appUser/findFunctionId?requestUser=sxbctv&requestPassword=123456',
                        dataType: 'json',
                        data: {functionCode: hongheibangdan},
                        success: function(data){
                            console.log(cateId);
                            var functionId = data.data.id;
                            sessionStorage.setItem('functionId',functionId);
                            System.startImgSelectAC(3,sessionStorage.getItem('itemId'),sessionStorage.getItem('appUserId'),cateId,sessionStorage.getItem('functionId'));
                        }
                    })
